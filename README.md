...
> **MythicMobs 5.x / MythicCrucible 視覺化編輯器** — 單一 HTML 檔案，免安裝、免網路，用瀏覽器直接讀寫伺服器設定檔。

![平台](https://img.shields.io/badge/平台-Chrome%20%2F%20Edge-blue)
![相依](https://img.shields.io/badge/相依-無（單檔離線可用）-green)
![支援](https://img.shields.io/badge/支援-MythicMobs%205.x%20%2F%20MythicCrucible-orange)
...
- **全中文技能資料庫** — 內建中文化的技能機制（Mechanics）、目標選擇器（Targeters）、條件（Conditions）、觸發器（Triggers）與怪物選項說明，寫技能不用一直翻英文 Wiki。
...
- **伺服器端**：安裝了 MythicMobs 5.x 的伺服器；物品編輯需另裝 MythicCrucible。
- 編輯器需要能直接存取伺服器檔案 —— 適合本機開服或有掛載遠端資料夾的環境。
...
**Q：打開後點「開啟資料夾」沒反應 / 顯示不支援？**
A：請確認使用 Chrome 或 Edge，其他瀏覽器沒有實作資料夾讀寫 API。

**Q：可以編輯遠端主機（VPS / 託管商）的伺服器嗎？**
A：工具需要直接存取檔案系統。若是遠端主機，可先把 `plugins/MythicMobs` 下載到本機編輯後再上傳，或使用 SFTP 掛載成本機磁碟。

**Q：存檔後遊戲內沒變化？**
A：記得執行 `/mm reload`。若仍無效，檢查後台是否有 YAML 解析錯誤訊息。
